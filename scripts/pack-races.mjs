...
const sourceDir = path.join(__dirname, '../src/packs/races');
const packDir = path.join(__dirname, '../packs/races');
...
async function packRaces() {
  ...
  try {
    ...
    console.log(`\n✓ Successfully packed ${count} races`);
  } catch (error) {
    console.error('Error packing races:', error);
    throw error;
  } finally {
    await db.close();
  }
